 ...
 */
const userEmail = "2387736781@qq.com";
const userCode = "whluhikkeiklecdb";
// 管理员获取借阅记录接口
...
router.post("/initreportlist", (req, res) => {
  conn.query(
    `select b.email,status,commentId,reporterId,
...
router.post("/reservelist", (req, res) => {
  conn.query(
    `select reader.readerId,book.bookId,readerName,bookName,date from reserve 
...
router.post("/searchborrow", (req, res) => {
  let data = req.body;
  conn.query(
    `select * from borrowinfo where borrowinfo.readerName='${data.info}'`,
    (err, reader) => {
      reader = reader || [];
      conn.query(
        `select * from borrowinfo where borrowinfo.bookName like '%${data.info}%'`,
        (err, book) => {
          book = book || [];
...
router.post("/adminaddbooks", (req, res) => {
  let data = req.body;
  conn.query(
    `select * from book where position = '${data.position}'`,
    (err, rs) => {
      if (rs.length > 0) {
        res.send({
          msg: "该位置已有书籍存放！",
        ...
        conn.query(
          `insert into book values('${nanoid()}','${data.bookName}','${
            data.author
          }','${data.amount}','${data.position}','${data.amount}',0,1)`
        );
        res.send({
          msg: "添加书籍成功！",
          status: 200,
...
router.post("/changebookinfo", (req, res) => {
  ...
  switch (status) {
    case "1": {
      // 修改书名
      conn.query(
        `update book set bookName='${data.value}' where bookId='${data.bookId}'`
      );
      res.send({
        status: 200,
        msg: "修改书名成功！",
    ...
    case "2": {
      // 修改作者
      conn.query(
        `update book set author='${data.value}' where bookId='${data.bookId}'`
      );
      res.send({
        status: 200,
        msg: "修改作者成功！",
    ...
    case "3": {
      // 修改位置
      conn.query(
        `select * from book where position = '${data.value}'`,
        (err, rs) => {
          if (rs.length > 0) {
            return res.send({
              msg: "该位置已有书籍存放！",
            ...
            conn.query(
              `update book set position='${data.value}' where bookId='${data.bookId}'`
            );
            res.send({
              status: 200,
              msg: "修改位置成功！",
    ...
    case "4": {
      // 修改当前库存
      conn.query(
        `update book set amount='${data.value}' where bookId='${data.bookId}'`
      );
      // 修改总库存
      conn.query(
        `update book set totalAmount=totalAmount + '${data.difference}' where bookId='${data.bookId}'`
      );
      res.send({
        status: 200,
        msg: "修改当前库存成功！",
...
router.post("/alertperson", (req, res) => {
  let data = req.body;
  conn.query(
    `select email from reader where readerId='${data.readerId}'`,
    (err, rs) => {
      console.log("rs:", rs);
...
router.post("/auditcomment", (req, res) => {
  ...
  if (data.status == 3) {
    conn.query(
      `update comment set status=0 where readerId='${data.readerId}' and bookId='${data.bookId}' and date='${data.date}'`
    );
    res.send({
      msg: "删评成功！",
      status: 200,
  ...
  //  删评
    ...
    conn.query(
      `update report set status='已通过' where commentId='${data.commentId}' and reporterId='${data.reporterId}' and reportdate='${data.reportdate}'`
    );
    // 邮件信息
    let mailobj = {
      from: userEmail, // 发送者QQ邮箱
    ...
    conn.query(
      `update report set status='已驳回' where commentId='${data.commentId}' and reporterId='${data.reporterId}' and reportdate='${data.reportdate}'`
    );
    // 邮件信息
    let mailobj = {
